...
import "@openzeppelin/contracts-upgradeable-4.2.0/token/ERC20/extensions/draft-ERC20PermitUpgradeable.sol";
 ...
 */
contract SRS is SimpleGovernance, ERC20PermitUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using MathUpgradeable for uint256;

    // Token max supply is 1,000,000,000 * 1e18 = 1e27
    uint256 public govCanUnpauseAfter;
    uint256 public anyoneCanUnpauseAfter;
    address public vestingContractTarget;
    ...
    mapping(address => bool) public allowedTransferTo;
    address public minter;

    event Allowed(address indexed target);
    event AllowedTo(address indexed target);
    event Disallowed(address indexed target);
    event DisallowedTo(address indexed target);
    ...
    function __SRS_init(
        ...
        __ERC20_init("Sirius Finance", "SRS");
        __ERC20Permit_init("Sirius Finance");
        ...
        allowedTransferTo[governance_] = true;
        ...
        emit SetGovernance(governance_, governance_);
    ...
    */
        ...
        returns (address)
    {
        ...
        Vesting vestingContract = Vesting(ClonesUpgradeable.clone(vestingContractTarget));
    ...
    function addTransferToList(address[] memory targets)
        external
        onlyGovernance
    {
        for (uint256 i = 0; i < targets.length; i++) {
            allowedTransferTo[targets[i]] = true;
            emit AllowedTo(targets[i]);
        }
    }
    
    function removeTransferToList(address[] memory targets)
        external
        onlyGovernance
    {
        for (uint256 i = 0; i < targets.length; i++) {
            allowedTransferTo[targets[i]] = false;
            emit DisallowedTo(targets[i]);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || allowedTransferee[from] || allowedTransferTo[to], "SRS: paused");
    ...
    function rescueTokens(
        ...
        if (token == IERC20Upgradeable(address(0))) {
            ...
            balance = balance == 0
                ? totalBalance
                : MathUpgradeable.min(totalBalance, balance);
