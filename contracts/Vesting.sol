 ...
 */
contract Vesting is OwnerPausableUpgradeable {
    ...
    uint256 public durationInSeconds; // duration after cliff
    uint256 public cliffRatio; // immediately release certain ratio of tokens after cliff
    ...
    function initialize(
        ...
        require(_cliffRatio <= 100 && _cliffRatio >= 0, "cliff ratio should be between 0 to 100");
    ...
    function vestedAmount() public view returns (uint256) {
        ...
        if (elapsedTime < cliffInSeconds) {
            return 0;
        } else if (elapsedTime >= cliffInSeconds + _durationInSeconds) {
            ...
            uint256 cliffRelease = totalBalance * cliffRatio / 100;
            uint256 linearRelease = totalBalance * (100 - cliffRatio) / 100;
            uint256 timeRatio = (elapsedTime - cliffInSeconds) * 100 / _durationInSeconds;
            if (released == 0) {
                // User hasn't claim the cliff release
                uint256 vested = linearRelease * timeRatio / 100 + cliffRelease;
                return vested;
            } else {
                uint256 vested = linearRelease * timeRatio / 100 + cliffRelease;
    ...
    function governance() public view returns (address) {
        return ISimpleGovernance(address(token)).governance();
    }
}
