...
contract Minter is OwnerPausableUpgradeable {
    ...
    event ToggleApproveMint(address sender, address mintingUser, bool approved);
    ...
    function mintFor(address _gaugeAddr, address _for) external nonReentrant() {
        require(allowedToMintFor[msg.sender][_for], "Minter: !for");
    ...
    function toggleApproveMint(address _mintingUser) external {
        ...
        emit ToggleApproveMint(msg.sender, _mintingUser, allowedToMintFor[_mintingUser][msg.sender]);
    }
