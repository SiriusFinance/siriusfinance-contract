...
contract MetaSwap is Swap {
    ...
    function initializeMetaSwap(
        ...
        // Read all tokens that belong to baseSwap
        {
            uint8 i;
            for (; i < 32; i++) {
                try baseSwap.getToken(i) returns (IERC20Upgradeable token) {
                    metaSwapStorage.baseTokens.push(IERC20Upgradeable(token));
        ...
        IERC20Upgradeable baseLPToken = _pooledTokens[_pooledTokens.length - 1];
        require(
