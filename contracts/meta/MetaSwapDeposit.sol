 ...
 */
contract MetaSwapDeposit is OwnerPausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMathUpgradeable for uint256;

    ISwap public baseSwap;
    IMetaSwap public metaSwap;
    IERC20Upgradeable[] public baseTokens;
    IERC20Upgradeable[] public metaTokens;
    IERC20Upgradeable[] public tokens;
    IERC20Upgradeable public metaLPToken;
    ...
    function initialize(
        ISwap _baseSwap,
        IMetaSwap _metaSwap,
        IERC20Upgradeable _metaLPToken
        ...
        // Check and approve base level tokens to be deposited to the base Swap contract
        {
            uint8 i;
            for (; i < 32; i++) {
                try _baseSwap.getToken(i) returns (IERC20Upgradeable token) {
        ...
        IERC20Upgradeable baseLPToken;
        {
            uint8 i;
            for (; i < 32; i++) {
                try _metaSwap.getToken(i) returns (IERC20Upgradeable token) {
                    baseLPToken = token;
    ...
    function addLiquidity(
        ...
        IERC20Upgradeable[] memory memBaseTokens = baseTokens;
        IERC20Upgradeable[] memory memMetaTokens = metaTokens;
        ...
        uint256 baseLPTokenAmount;
        {
            ...
            for (uint8 i = 0; i < memBaseTokens.length; i++) {
                IERC20Upgradeable token = memBaseTokens[i];
        ...
        uint256 metaLPTokenAmount;
        {
            ...
            for (uint8 i = 0; i < baseLPTokenIndex; i++) {
                IERC20Upgradeable token = memMetaTokens[i];
    ...
    function removeLiquidityImbalance(
        ...
        if (v.withdrawFromBase) {
            ...
            IERC20Upgradeable baseLPToken = memMetaTokens[v.baseLPTokenIndex];
