...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const META_POOL_NAME = "SiriusUSDSMetaPool"
  const META_POOL_LPTOKEN_NAME = `${META_POOL_NAME}LPToken`
  const BASE_POOL_NAME = "SiriusUSDPool"

  const TOKEN_ADDRESSES = [
    (await get("USDs")).address,
    (await get("SiriusUSDPoolLPToken")).address,
  ]
  const TOKEN_DECIMALS = [18, 18]
  const LP_TOKEN_NAME = "Sirius USD Metapool: USDs-siriusUSD"
  const LP_TOKEN_SYMBOL = "USDs-siriusUSD"
  const INITIAL_A = 100
  ...
  if (metaPool) {
    ...
    log(`Sirius ${META_POOL_LPTOKEN_NAME} at ${lpTokenAddress}`)
