...
// Filters
function siriusFilesFilter(file) {
  const name = file.toLowerCase()
  return name.includes("sirius") || name.includes("sdl")
}
...
/**
 * @notice Prints all Sirius contract addresses parsed from the deployments directory
...
function main() {
  ...
  if (args["--help"]) {
    console.log(
      `Usage: node printSiriusAddrs.js [--network=<network>] [--only-addresses] [--only-pools] [--json] [--help]`,
    )
    return
  }
