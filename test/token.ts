...
import { SRS, Vesting } from "../build/typechain/"
...
describe("Token", () => {
  ...
  describe("minting", () => {
    it("Successfully mints to appropriate addresses and vestings", async () => {
      ...
      expect(await siriusToken.balanceOf(await signers[10].getAddress())).to.eq(
        0,
      )
      expect(await siriusToken.balanceOf(await signers[11].getAddress())).to.eq(
        0,
      )
      expect(await siriusToken.balanceOf(await signers[12].getAddress())).to.eq(
        0,
      )
      expect(await siriusToken.balanceOf(vestingContracts[0].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await siriusToken.balanceOf(vestingContracts[1].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await siriusToken.balanceOf(vestingContracts[2].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )

      // Non-vesting addresses
      expect(await siriusToken.balanceOf(await signers[13].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await siriusToken.balanceOf(await signers[14].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
    })
  })
  ...
  describe("totalSupply", () => {
    it("Successfully mints max supply (1e9 with 1e18 decimals) on deployment", async () => {
      expect(await siriusToken.totalSupply()).to.eq(BIG_NUMBER_1E18.mul(1e9))
    })
  })

  describe("govCanUnpauseAfter", () => {
    it("Successfully sets govCanUnpauseAfter to be in the future on deployment", async () => {
      expect(await siriusToken.govCanUnpauseAfter()).to.eq(
  ...
  describe("anyoneCanUnpauseAfter", () => {
    it("Successfully sets anyoneCanUnpauseAfter to be 1 year after the deployment", async () => {
      expect(await siriusToken.anyoneCanUnpauseAfter()).to.eq(
  ...
  describe("governance", () => {
    it("Successfully sets the governance address on deployment", async () => {
      expect(await siriusToken.governance()).to.eq(governanceAddress)
    })
  })
  ...
  describe("transfer", () => {
    it("Successfully transfers from an allowed address when paused", async () => {
      ...
      expect(await siriusToken.balanceOf(deployerAddress)).to.eq(
    ...
    it("Reverts when transfers from a not-allowed address to an allowed address when paused", async () => {
      await siriusToken
        ...
        .transfer(deployerAddress, BIG_NUMBER_1E18.mul(1e8))
      await expect(
        siriusToken.transfer(
          await signers[13].getAddress(),
          BIG_NUMBER_1E18.mul(1e8),
        ),
      ).to.be.revertedWith("SRS: paused")
    })
  ...
  describe("enableTransfer", () => {
    it("Reverts when governance attempts to unpause before govCanUnpauseAfter", async () => {
      expect(await siriusToken.govCanUnpauseAfter()).to.gt(
        await getCurrentBlockTimestamp(),
      )
      await expect(
        siriusToken.connect(governance).enableTransfer(),
      ).to.be.revertedWith("SRS: cannot enable transfer yet")
    })

    it("Reverts when non-governance attempts to unpause after govCanUnpauseAfter", async () => {
      await setTimestamp(
        (await siriusToken.govCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await siriusToken.govCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await expect(
        siriusToken.connect(deployer).enableTransfer(),
      ).to.be.revertedWith("SRS: cannot enable transfer yet")
    })
    ...
    it("Succeeds when governance attempts to unpause after govCanUnpauseAfter", async () => {
      ...
      await siriusToken.connect(governance).enableTransfer()
      expect(await siriusToken.paused()).to.be.false
    })

    it("Succeeds when non-governance attempts to unpause after anyoneCanUnpauseAfter", async () => {
      await setTimestamp(
        (await siriusToken.anyoneCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await siriusToken.anyoneCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await siriusToken.enableTransfer()
      expect(await siriusToken.paused()).to.be.false
    })
    ...
    it("Reverts when attempting to call enableTransfer after it is already unpaused", async () => {
      ...
      await expect(
        siriusToken.connect(deployer).enableTransfer(),
      ).to.be.revertedWith("SRS: transfer is enabled")
    })
    ...
    describe("transfer after enableTransfer", () => {
      ...
      it("Succeeds when transferring between not-allowed addresses", async () => {
        ...
        expect(
          await siriusToken.balanceOf(await signers[16].getAddress()),
  ...
  describe("addToAllowedList", () => {
    it("Succeeds to add an address to the allowed list", async () => {
      ...
      await siriusToken.connect(governance).addToAllowedList([deployerAddress])
    ...
    it("Reverts when called by non-governance", async () => {
      await expect(
        siriusToken.connect(deployer).addToAllowedList([deployerAddress]),
  ...
  describe("changeGovernance & acceptGovernance", () => {
    it("Reverts when called by other than the governance", async () => {
      await expect(
        siriusToken.connect(deployer).changeGovernance(deployerAddress),
      ).to.be.revertedWith("only governance can perform this action")
    })

    it("Succeeds to change governance", async () => {
      await siriusToken.connect(governance).changeGovernance(deployerAddress)
      expect(await siriusToken.governance()).to.be.eq(governanceAddress)
      await siriusToken.connect(deployer).acceptGovernance()
      expect(await siriusToken.governance()).to.be.eq(deployerAddress)
      expect(await siriusToken.pendingGovernance()).to.be.eq(ZERO_ADDRESS)
    })

    it("Reverts when accepting governance when changeGovernance is not called before", async () => {
      await expect(
        siriusToken.connect(deployer).acceptGovernance(),
    ...
    it("Reverts when accepting governance when called by other than pendingGovernance", async () => {
      await siriusToken.connect(governance).changeGovernance(deployerAddress)
      expect(await siriusToken.pendingGovernance()).to.be.eq(deployerAddress)
      await expect(
        siriusToken.connect(signers[13]).acceptGovernance(),
  ...
  describe("rescueToken", () => {
    ...
    it("Successfully rescues ERC20", async () => {
      ...
      await dummyToken.mint(siriusToken.address, BIG_NUMBER_1E18.mul(10000))

      expect(await dummyToken.balanceOf(siriusToken.address)).to.eq(
