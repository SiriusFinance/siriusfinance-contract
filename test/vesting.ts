...
describe("Vesting", () => {
  ...
  describe("vestedAmount", () => {
    ...
    describe("contract is initialized and filled with some tokens", () => {
      ...
      it("Successfully calculates the vested amounts - before cliff reached", async () => {
        const startTimestamp = await vestingClone.startTimestamp()
        expect(await vestingClone.vestedAmount()).to.eq(0)
      ...
      it("Successfully calculates the vested amounts - cliff just reached", async () => {
        const startTimestamp = await vestingClone.startTimestamp()
        await setTimestamp(startTimestamp.add(3600).toNumber())
        expect(await vestingClone.vestedAmount()).to.eq(
          totalVestedAmount.div(5), // 20% release after cliff
        )
      })

      it("Successfully calculates the vested amounts - cliff reached, duration not over", async () => {
        const startTimestamp = await vestingClone.startTimestamp()
        await setTimestamp(startTimestamp.add(5400).toNumber())
        expect(await vestingClone.vestedAmount()).to.eq(
          totalVestedAmount.mul(60).div(100), // 20% + 80% / 2
        )
      })

      it("Successfully calculates the vested amounts - after duration", async () => {
        const startTimestamp = await vestingClone.startTimestamp()
  ...
  describe("release", () => {
    ...
    it("Successfully releases the vested amounts", async () => {
      ...
      await setTimestamp(startTimestamp.add(3600).toNumber())
      expect(await vestingClone.vestedAmount()).to.eq(totalVestedAmount.div(5))
      await vestingClone.connect(beneficiary).release()
      expect(await dummyToken.balanceOf(beneficiaryAddress)).eq(totalVestedAmount.div(5))


      await setTimestamp(startTimestamp.add(5400).toNumber())
      await vestingClone.connect(beneficiary).release()
      expect(await dummyToken.balanceOf(beneficiaryAddress)).eq(totalVestedAmount.mul(60).div(100))
